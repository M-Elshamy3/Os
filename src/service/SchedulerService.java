...
public class SchedulerService {
    ...
    public ScheduleOutput runPreemptiveSJF(List<Process> processes) {
        ...
        rows.sort(Comparator.comparing(ResultRow::getPid));
            ...
            for (int i = 0; i < n; i++) {
                if (!done[i] && processes.get(i).getArrival() <= time && remaining[i] > 0) {
                    if (chosen == -1) {
                        ...
                        if (processes.get(i).getPriority() < processes.get(chosen).getPriority()) {
                            chosen = i;
    ...
    private void appendSegment(List<Segment> gantt, String pid, int start, int end) {
        if (!gantt.isEmpty() && gantt.get(gantt.size() - 1).getPid().equals(pid) && gantt.get(gantt.size() - 1).getEnd() == start) {
